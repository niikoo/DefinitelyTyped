...
declare namespace Base {
  ...
  /**
   * Interface for objects that can their data as a Blob.
  ...
  interface UserMethods {
     ...
     * The circumstances in which the email address is available vary: for example,
     * the user's email address is not available in any context that allows a script
     * to run without that user's authorization, like a simple `onOpen(e)` or `onEdit(e)` trigger,
     * a custom function in Google Sheets, or a web app deployed to "execute as me"
     * (that is, authorized by the developer instead of the user). However, these
     * restrictions generally do not apply if the developer runs the script
     * themselves or belongs to the same G Suite domain as the user.
     * @returns The user's email's address, or a blank string if security policies do not allow access to the user's email address.
     */
    getEmail(): string;
    getUsername(): string;
    toString(): string;
...
declare const Browser: Base.Browser;
declare const Logger: Base.Logger;
declare const MimeType: Base.MimeType;
declare const Session: Base.Session;